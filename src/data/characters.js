...
export const CHARACTERS = [
  {
    ...
    evolutions: [
      { atLevel: 10, id: 'quagsire', name: 'Quagsire', palette: 'quagsire',
        grant: { maxHp: +70, moveSpeed: +6, armor: +2, area: +0.10, regen: +0.40 },
        note: 'Mud Shot gains +1 glob and wider puddles.' },
      { atLevel: 20, id: 'quagsire_unaware', name: 'Quagsire', palette: 'quagsire',
...
export function characterSpritePairs() {
  ...
  for (const c of CHARACTERS) {
    out.push([c.shape, c.palette]);
    for (const ev of c.evolutions) {
      if (ev.palette) out.push([c.shape, ev.palette]);
      if (ev.branch) for (const b of ev.branch) out.push([c.shape, b.palette]);
    }
  }
  return out;
}
